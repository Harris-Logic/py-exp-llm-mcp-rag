...
from mcp import McpError, ServerResult
...
class Agent:
    ...
    async def init(self):
        ...
        self.llm = try_chat_openai.AsyncChatOpenAI(
            self.model_name, 
            self.system_prompt, 
            tools,
        )
            ...
            if server_response.toolCalls is not None:
                for tool_call in server_response.toolCalls:
                    ...
                    if mcp is not None:
                        ...
                        utils.pretty.log_title(f"with args:", tool_call.function.arguments)

                        mcp_result = await mcp.call_tools(
                            tool_call.function.name,
                            json.loads(tool_call.function.arguments),
                        )
                        # utils.pretty.log_title(f"result: `{json.}`")
                        utils.pretty.log_title(f"result: ", str(mcp_result))

                        self.llm.append_tool_result(
                            tool_call_id=tool_call.id, 
                            # tool_output=mcp_result.model_dump_json(),
                            tool_output=json.dumps(mcp_result.model_dump()),
                        )
                    else:
                        self.llm.append_tool_result(
                            tool_call.id,
